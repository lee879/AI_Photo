...
from res_attion_assist import ResNet_Discriminator,ResNet_Generator
...
def g_loss_fn(generator,discriminator,batch_z,batch_xy,is_training):
    fake_img = generator(batch_z,training=is_training)
...
def main():
    ...
    for epoch in range(epochs):
        ...
        tf.optimizers.Adam(learning_rate=0.001, beta_1=0.9).apply_gradients(zip(grads, discriminator.trainable_variables))

        # if tf.math.abs(d_loss - temp_d_loss) <= 10: #loss约束
        with tf.GradientTape() as tape:
            g_loss, fake_img = g_loss_fn(generator, discriminator, batch_z, batch_xy, is_training)
        grads = tape.gradient(g_loss, generator.trainable_variables)
        tf.optimizers.Adam(learning_rate=0.001, beta_1=0.9).apply_gradients(zip(grads, generator.trainable_variables))
